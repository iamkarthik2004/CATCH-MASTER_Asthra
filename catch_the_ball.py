...
class Game:
    ...
    def __init__(self):
        ...
        self.high_score = 0
        self.level = 1
        self.lives = self.config.max_lives  # Initialize with 3 lives
    ...
    def reset_game(self):
        ...
        self.score = 0
        self.level = 1
        self.lives = self.config.max_lives  # Reset to 3 lives
        self.game_over = False
        self.paused = False
    ...
    def update_game(self):
        ...
        for ball in self.balls[:]:
            ...
            if (self.pot.y <= ball.y + ball.radius <= self.pot.y + self.pot.height and 
                ...
                self.balls.remove(ball)
                self.lives -= 1  # Lose a life when a ball is missed
                logging.info(f"Ball missed! Lives remaining: {self.lives}")
                if self.lives <= 0:
                    self.game_over = True
    ...
    def render_game(self):
        ...
        if self.current_state == "menu":
            ...
            lives_text = self.resources.render_text(f"Lives: {self.lives}", "medium", self.config.colors["RED"])
            self.window.blit(lives_text, [10, 90])  # Display lives below level
            ...
            self.window.blit(
                lives_text,
                [self.config.width // 2 - lives_text.get_width() // 2, self.config.height // 2 + 60]
            )
            self.window.blit(
                restart_text,
                [self.config.width // 2 - restart_text.get_width() // 2, self.config.height // 2 + 100]
            )
